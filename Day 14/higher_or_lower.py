...
while game_state["continue"]:
    ...
    if user_choice == "a":
        if game_state["a"]["follower_count"] >  game_state["b"]["follower_count"]:
            game_state["current_score"] += 1
            print(f"You're right! Current score: {game_state["current_score"]}.")
            if len(datasets) > 0:
                datasets.remove(game_state["a"])
                game_state["a"] = game_state["b"]
            else:
                print(f"You're won!.")
        elif game_state["a"]["follower_count"] < game_state["b"]["follower_count"]:
            print(f"Sorry, that's wrong. Final score: {game_state["current_score"]}.")
            game_state["continue"] = False
    elif user_choice == "b":
        if game_state["b"]["follower_count"] >  game_state["a"]["follower_count"]:
            game_state["current_score"] += 1
            print(f"You're right! Current score: {game_state["current_score"]}.")
            if len(datasets) > 0:
                datasets.remove(game_state["b"])
                game_state["b"] = game_state["a"]
            else:
                print(f"You're won!.")
        elif game_state["b"]["follower_count"] < game_state["a"]["follower_count"]:
            print(f"Sorry, that's wrong. Final score: {game_state["current_score"]}.")
            game_state["continue"] = False
